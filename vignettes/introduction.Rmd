...
load(file=paste0(datapath, 'ebdat.rda'))
ebdatsub <- ebdat[,c(58:413)]

rm(ebdat) ## only keep the subset that we will work with
...
```
We also need to remove the anchor points, which are the synthetic points that we added to the data to set the scale of the estimated dimensions.
...
```{r, eval=F, echo=T}
 eval=F, echo=T, message=F, warning = FALSE}
library(ggplot2) #version: ggplot2_3.4.4 
...
ggsave(gg, file="ebirtm.png", width=5, height=5, units ="in", dpi=300)
```

![Visualization of estimated Theta distributions, Eurobarometer Data](ebirtm.png){width=50%}
