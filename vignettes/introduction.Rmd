...
## Loading the package
Gfortran will be a necessary component in loading the package. Gfortran can be readily downloaded and the version installed can be checked: On Windows, enter into the command line "$ gfortran --version GNU Fortran." On Mac, enter into the terminal "which gfortran." After gfortran has been successfully installed, it is important to also have GCC (GNU Compiler Collection) installed. 
...
# Combining the M-Matrix with the observed data for IRT-M modeling
...
# Creating a 3D array for M-Matrix where each matrix corresponds to a coded loading question
...
The code below will run the sampler on the formatted data (the code that follows has a toy burn-in and sampler runtime; a real runtime would be closer to 10^3- 10^4 iterations with a 2,000 burn-in).
...
![Visualization of estimated Theta distribution.](theta_dist.png){width=50%}
...
![Visualization of the distribution of Thetas by media trust.](theta_media_trust.png){width=50%}
...
lambda_weight <- as.data.frame(irt$lambda[,theta_id,]) %>%
   ...
   mutate(mean = mean(c_across(starts_with("V")), na.rm = TRUE),
          var = variance(c_across(starts_with("V")), na.rm = TRUE)) %>%
  select(mean, var) %>%
  ungroup() 
```

The result is a dataframe with the mean and variance of the `lambda` distribution of the selected underlying dimension identified for each of our one-hot encoded questions. At this point, it is convenient to go to the `M-Code` data file so that we can attach the human-readable question names. To do this, we read in the `.csv` file where we did the original coding. As when we prepared the data, we keep only those rows that have a loading coded:
...
In this case, we coded the `MCodes` object with a column that provides the question number and another that provides substantive notes on what the question was asking. We can extract those columns and attach them to the `lambda` summary:
...
Once we have this data, we can model the questions and lambda loadings to check for internal validity of the model. 
...
We can repeat this analysis for additional dimensions to gain insight into the questions that contribute the most to the estimation of the dimension. For example, doing so for our `\Theta_{1}` ("culture threat"), we can learn that the question which loads most heavily is `qa6b_9_2` ("Do not trust national parliament").
